...
class MC(commands.Cog):
    ...
    async def create(self, ctx: Context, name: str, mctype="vanilla", *, args="0"):
        ...
        if MCBotScripts.blacklisted(str(ctx.author.id)):
            await ctx.send("You were blacklisted by the bot owner")
            return
        if MCBotScripts.get(ctx, name):
        ...
        currentactive = len(
            self.db.search(
                (querycheck.owner == ctx.author.id)
                & (querycheck.guildId == ctx.guild.id)
            )
        )
        if currentactive >= maxperuser:
            if str(ctx.author.id) not in botowners:
                await ctx.send(
                    f"You can only have {maxperuser} servers active per person"
                )
                return
        if (
            len(
        ...
        if (
            len(
                self.db.search(
                    (querycheck.status == "up") & (querycheck.guildId == ctx.guild.id)
                )
            )
            >= maxservers
        ):
            if str(ctx.author.id) not in botowners:
                await ctx.send(
                    f"This server can only have {maxservers} servers up at once"
                )
                return
        message = await ctx.send(
            "Attempting to create the world (may take a few minutes if you used a link)..."
        )
        mainenv = {
            "VERSION": None,
            "MAX_MEMORY": None,
            "MOTD": "A minecraft server created by the Voark bot",
        }
            ...
            for a in argslist:
                ...
                if temp[0] == "MAX_MEMORY":
                    try:
                        ...
                        await ctx.send(
                            "Memory can only be set between 2 and 8 gigabytes. Ask the bot owner if you need more"
                        )
                        return
                elif temp[0] == "VERSION":
        ...
        if mctype.lower() == "vanilla":
            ...
            (mctype.lower() == "custom")
            or (mctype.lower() == "paper")
            or (mctype.lower() == "bukkit")
            or (mctype.lower() == "spigot")
        ):
            linkpropenv = dict.fromkeys(["DATAPACKS", "ICON", "WORLD"], None)
            otherpropenv = dict.fromkeys(
                ["VANILLATWEAKS_SHARECODE", "SPIGET_RESOURCES"], None
            )
            seed = None
        ...
        await MCBotScripts.startup(ctx, processname, finalip)
        self.db.update(
            {"status": "up"},
            (where("serverId") == ctx.guild.id) & (where("name") == name),
        )
    ...
    async def set(self, ctx: Context, name: str, *, args):
        if MCBotScripts.blacklisted(str(ctx.author.id)):
            await ctx.send("You were blacklisted by the bot owner")
            return
        if not MCBotScripts.get(ctx, name):
            await ctx.send("Server with this name does not exist.")
            return
        if not MCBotScripts.perms(ctx, name):
        ...
        env = {
            "intpropenv": intpropenv,
            "strpropenv": strpropenv,
            "boolpropenv": boolpropenv,
            "linkpropenv": linkpropenv,
            "otherpropenv": otherpropenv,
            "maxmemory": maxmemory,
        }
            ...
            for a in arglist:
                try:
                    ...
                    if temp[0] in intpropenv:
                        ...
                        await ctx.send(
                            f"Did not find {temp[0]} in valid environment variables. Please try again"
                        )
                        return
                except KeyboardInterrupt:
                    await ctx.send(
                        "Error reading values. Check your spelling and try again"
                    )
                    return
    ...
    async def delete(
        ...
        *,
        ...
        await MCBotScripts.serverstop(ctx, name)
        with PodmanClient(base_url=uri) as client:
            ...
            if keepworld.lower() == "true":
                ...
                self.backups.insert(
                    {
                        "volume": {
                            "name": volumepath.split("/")[
                                volumepath.split("/").index("volumes") + 1
                            ],
                            "Source": volumepath,
                        },
                        "worldname": worldname,
                ...
                try:
                    await ctx.send(
                        "Are you sure you want to delete the world (There is no going back):\nTo confirm type (y)es. If no, type anything else"
                    )
                    ...
                    await response.edit(
                        "You did not confirm in time. Cancelling deletion"
                    )
                    self.db.update(
                        {"status": "down"},
                        (where("serverId") == ctx.guild.id) & (where("name") == name),
            ...
            try:
                await cloudscript.delete(name, ctx.guild.id)
            except Exception as e:
                print(e)
            self.db.remove(
                (where("serverId") == ctx.guild.id) & (where("name") == name)
    ...
    async def transfer(self, ctx: Context, name: str, newowner: str):
        ...
        currentactive = len(
            self.db.search(
                (querycheck.owner == owner) & (querycheck.guildId == ctx.guild.id)
            )
        )
        if currentactive >= maxperuser:
        ...
        if (
            len(
                self.backups.search(
                    (where("guildId") == ctx.guild.id) & (where("owner") == owner)
                )
            )
            + currentactive
            >= maxworlds
        ...
        try:
            await ctx.send(
                "Are you sure you want to transfer the world?\nTo confirm type (y)es. If no, type anything else"
            )
        ...
        try:
            message = await ctx.send(
                f"Are you sure you want to accept the ownership? To confirm, have THEM type (y)es. If no, type anything else. \nCancelling in 30 seconds"
            )
            response = await self.bot.wait_for("message", check=check1, timeout=30.0)
        except asyncio.TimeoutError:
            await message.edit(
                content=f"<@{owner}> did not reply in time. Please try again"
            )
            return
        if response.content.lower() not in ("yes", "y"):
    ...
    async def restore(self, ctx: Context, worldname: str, *, args):
        ...
        for a in arglist:
            ...
            if temp[0].lower() == "version":
                ...
                if MCBotScripts.get(ctx, temp[1]):
                    await ctx.send("Server with name already exists.")
                    return
                name = temp[1]
            else:
